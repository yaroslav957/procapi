...
use std::{io::{Error, ErrorKind}, ptr::null_mut};
    ...
    fn try_from(pid: i32) -> Result<Self, Self::Error> {
        if let Ok(info) = proc_pid::pidinfo::<TaskAllInfo>(pid, 0) {
            ...
            Ok(Process {
                ...
                name: proc_pid::name(pid)
                    .unwrap_or_else(|_| pidpath(pid).unwrap_or_default()),
                state: State::try_from(pth_state)?,
                cmd: get_cmdline(pid as u32).unwrap_or_else(|| {
					vec![pidpath(pid).unwrap_or_default()]
				}).join(" "),
            })
        } else {
...
fn get_cmdline(pid: u32) -> Option<Vec<String>> {
    ...
    let process_args: *mut u8 = unsafe {
        std::mem::transmute(
            libc::calloc(size, std::mem::size_of::<size_t>())
        )
    };
    let mut res = Vec::<String>::new();

    assert_ne!(process_args, null_mut(), "process args buffer allocation failed");
    ...
    unsafe {
        ...
        let mut ch_ptr: *mut u8 = process_args.add(
...
unsafe fn get_str_unchecked(start: *mut u8, end: *mut u8) -> String {
